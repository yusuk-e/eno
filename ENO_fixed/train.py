...
def train():
    ...
    input_dim = 2 + vs['Nx']
    model = ENO_fixed(input_dim, args.hidden_dim, args.Hamilton, device, args.col_size, args.G).to(device)
...
if __name__ == "__main__":
    ...
    if args.Hamilton:
        save_dir = tmp_dir + '/' + str(args.s) + '/ENO_fixed/st_' + str(args.setting_no) + '/' + str(args.lam)
    else:
