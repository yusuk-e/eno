...
def get_args():
    ...
    parser.add_argument('--lr_H', default=1e-3, type=float, help='learning rate')
...
def train():
    ...
    input_dim = 2 + vs['Nx']
    model = HNO(input_dim, args.hidden_dim, args.Hamilton, device, args.col_size, args.G).double().to(device)
    ...
    for epoch in range(args.epochs+1):            
        ...
        for ids in batch_ids:
            loss, d_loss, h_loss = model.loss(t, x, u[ids], args.lam)
...
if __name__ == "__main__":
    ...
    if args.Hamilton:
        save_dir = tmp_dir + '/' + str(args.s) + '/HNO/st_' + str(args.setting_no) + '/' + str(args.lam)
    else:
    ...
    # training data
    u = torch.tensor(data['u']).squeeze().to(device)
    t = torch.tensor(data['t']).to(device)
    x = torch.tensor(data['x']).to(device)

    # validation data
    val_u = torch.tensor(data['val_u']).squeeze().to(device)
