...
class kdv():
    ...
    def get_init(self, N, x, X):
        ...
        u0 = []
        D1 = 0.3#np.random.uniform(0.2, 0.21, ndata)
        D2 = 0.3#np.random.uniform(0.2, 0.21, ndata)
        #np.random.seed(1)
        for i in range(N):
            #k1, k2 = np.random.uniform(#0.5, 2.0, 2)
            k1 = np.random.uniform(0.5, 1.0)#0.8, 1.0)
            k2 = np.random.uniform(1.5, 2.0)#1.8, 2.0)
            d1 = D1
...
def visualization_comp(save_dir, ts, us, es, ms, pred_us, pred_es, pred_ms, end_t, end_x, name):
    ...
    #cmap = cmo.delta#curl#balance
    import seaborn as sns
    cmap = sns.palplot(sns.diverging_palette(220, 20, n=24))
    cmap = 'seismic'#'vlag'#'twilight_shifted'
    for i in range(len(us)):
        ...
        #vmin = u.min() if u.min() < p_u.min() else p_u.min() #kdv
        #vmax = u.max() if u.max() > p_u.max() else p_u.max() #kdv
        if name == 'kdv':
            vmax = 5.2
            A = 4#10
        elif name == 'ch':
