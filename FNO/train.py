...
def get_args():
    ...
    parser.add_argument('--dh', default=200, type=int, help='hidden dimension of decoder')
...
def train():
    ...
    input_dim = vs['Nx']
    mode = int(vs['Nx']/args.mode)+1
    model = FNO(input_dim, args.hidden_dim, args.Hamilton, device, args.col_size, mode, args.nonlinearity, args.dh).to(device)
...
if __name__ == "__main__":
    ...
    save_dir = tmp_dir + '/' + str(args.s) + '/FNO/st_' + str(args.setting_no)
    ...
    # validation data
    val_u = torch.tensor(data['val_u'], dtype=torch.float).squeeze().to(device)
