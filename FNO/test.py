...
if __name__ == "__main__":
    ...
    i_dir = tmp_dir + '/FNO/st_' + str(args.setting_no)
    save_dir = i_dir + '/test'
    ...
    # test data
    u = torch.tensor(data['u'], dtype=torch.float).squeeze().to(device)
    t = torch.tensor(data['t'], dtype=torch.float).to(device)
    x = torch.tensor(data['x'], dtype=torch.float).to(device)
    e = data['e']
    m = data['m']
    dx = vs['X'] / vs['Nx']
    ...
    tmp = u[:,0,:]
    u0 = tmp[:,x_ids]
    train_x = torch.tensor(train_vs['X'] * np.arange(train_vs['Nx']) / (train_vs['Nx']-1), dtype=torch.float).to(device)

    # learned model
    json_open = open(i_dir + '/model.json', 'r')
    json_model = json.load(json_open)
    input_dim = train_vs['Nx']
    hidden_dim = json_model['hidden_dim']
    mode = json_model['mode']
    mode = int(train_vs['Nx']/mode)+1
    dh = json_model['dh']
    Hamilton = json_model['Hamilton']
    col_size = json_model['col_size']
    nl = json_model['nonlinearity']
    model = FNO(input_dim, hidden_dim, Hamilton, device, col_size, mode, nl, dh).to(device)
    ...
    with torch.no_grad():
        pred_u = []
        for tmp in u0:
            out = model.solve(t, x, tmp.unsqueeze(0))
            pred_u.append(out)
